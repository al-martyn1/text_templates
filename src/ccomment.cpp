            ...
            for(int lineNo = startLine; lineNo!=endLine; ++lineNo)
               {
                ...
                str = appendWhiteSpaces(str, endPos); // NOTE: лишний пробел в конце строки скорее всего добавлялся тут
            ...
            if (::Info.EditorCtrlGetString(endLine-1, str, eol))
               {
                if (endPos==-1)
                ...
                else
                   {
                    ...
                    str = appendWhiteSpaces(str, pos); // NOTE: лишний пробел в конце строки скорее всего добавлялся тут
...
//-----------------------------------------------------------------------------
void SelectOrRemoveComment(const EditorInfo &_edinfo, bool selected, 
                 int startLine, int startPos,
...
void SelectOrRemoveComment(const EditorInfo &_edinfo, bool selected, 
                 ...
                 int endLine, int endPos, bool bEraseDontSelect)
   {
    if (!_edinfo.FileName) return;

    EditorInfo edinfo = _edinfo;

    //SS;

    // NOTE: При выделенном в редакторе фрагменте,если позиция курсора 
    // находится за пределами закоментированного блока,коментарий не удаляется
    if (bEraseDontSelect && selected)
       {
        //ss<<"bEraseDontSelect && selected\n"
        //    "before, edinfo.CurPos: "<<edinfo.CurPos<<", edinfo.CurLine: "<<edinfo.CurLine<<"\n";
        edinfo.CurPos  = startPos;
        edinfo.CurLine = startLine;
        //ss<<"after, edinfo.CurPos: "<<edinfo.CurPos<<", edinfo.CurLine: "<<edinfo.CurLine<<"\n";
       }
    ...
    if (findSelectLinesBlockComment( edinfo, commentSequences, bEraseDontSelect))
       {
        //ss<<"findSelectLinesBlockComment\n";
        //Warning(IDM_COMMENTING_CODE, ss.str());
...
void selectCommentedText(const EditorInfo &edinfo, bool selected, 
                 ...
                 int endLine, int endPos)
   {
    //Warning(IDM_COMMENTING_CODE, "selectCommentedText");
...
void uncommentText(const EditorInfo &edinfo, bool selected, 
                 ...
                 int endLine, int endPos)
   {
    //Warning(IDM_COMMENTING_CODE, "uncommentText 1");
    if (!selected) return;
    //Warning(IDM_COMMENTING_CODE, "uncommentText 2");
    SelectOrRemoveComment(edinfo, selected, 
                          startLine, startPos,
                          endLine, endPos, 
...
void incrementalSelection(const EditorInfo &edinfo, bool selected, 
                 ...
                 int endLine, int endPos)
   {
    ...
    if (!selected || startLine!=edinfo.CurLine)
       {
        startLine = edinfo.CurLine;
        endPos = startPos  = edinfo.CurPos;
        ::Info.EditorCtrlSelect( BTYPE_NONE, 0, 0, 0, 0);
        //ss<<"endPos = startPos  = edinfo.CurPos;\n";
       }
    ...
    if (Utils::isSpaceOnlyString(curLine))
       {
        //Warning(IDM_COMMENTING_CODE, "2");
        return;
       }
    ...
    // SS;
    // ss<<"Line before expand tabs: ["<<curLine<<"]\n";
    // expandTabs(curLine, edinfo);
    // ss<<"Line after  expand tabs: ["<<curLine<<"]\n";
    ...
    if (selected)
       { // check is needed full line select 
        ...
        //for(; endPos<(int)curLine.size(); ++endPos)
        //   if (curLine[endPos]==' ') break;
        //for(; startPos>=0; --startPos)
        //   if (curLine[startPos]==' ') break;
        if (startPos<0) startPos = 0;
        if (!startPos && endPos>=(int)curLine.size())
           { // select full line
            ::Info.EditorCtrlSelect( BTYPE_STREAM, startLine, 
                                     0, -1, 
                                     1);
       ...
       {
        //for(; startPos<(int)curLine.size(); ++startPos)
        //   if (curLine[startPos]==' ') break;
        if (startPos>=(int)curLine.size())
           { // select full line
            ::Info.EditorCtrlSelect( BTYPE_STREAM, startLine, 
                                     0, -1, 
                                     1);
    ...
    CLineAstParser parser(breakSymbols, curLine);
    parser.buildAst();
    parser.incrementalSelect(startPos, endPos, (int)curLine.size());
    ::Info.EditorCtrlSelect( BTYPE_STREAM, startLine, 
