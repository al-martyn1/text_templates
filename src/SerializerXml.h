...
public:
    ...
    //INamedNodeList* ThisAsNamedNodeList() const;
...
struct XmlStorageException : public StorageException
{
    ...
    XmlStorageException(const XmlStorageException &e)
        : StorageException(e)
    {
    }

    XmlStorageException& operator=(const XmlStorageException &e)        
    {
        StorageException::operator=(e);
        return *this;
    }
};
//----------------------------------------------------------)
         ...
         )
{
    ...
    if (FAILED(pDoc.CoCreateInstance(L"Msxml2.DOMDocument")))
        throw XmlStorageException("Msxml2.DOMDocument creation failed. COM initialized?");

    if (FAILED(pDoc->put_async(VARIANT_FALSE)))
        throw XmlStorageException("pDoc->put_async");
    ...
    if (FAILED(pDoc->get_documentElement(&pRootElement)))
        throw XmlStorageException("Failed to get documnt object");

    CComBSTR nameRoot;
    if (FAILED(pRootElement->get_nodeName(&nameRoot)))
        throw XmlStorageException("Get root node name failed");

    if (CComBSTR(rootTag.c_str()) != nameRoot)
        throw XmlStorageException(tstring(_T("Incorrect type of xml documet. Root tag is not a ")) + rootTag );
